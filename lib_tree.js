  ...
  var newUlItem = document.createElement("ul");
    ...
    if (ul_hidden)
      newUlItem.className = "hide_ul";
                                    // calculate mean value of all evaluation values
  var eval_value = 0.0;
  for (var i=0; i<this.eval_cat_num; i++)
  { 
...
function print_disptype_tree(tree_obj, sel_elem_id, selected_item_in_tree)
{
  ...
  for (var i=start_idx; i<tree_obj.tree_nodes.length; i++)
  {
    ...
    this.print_item(parent_gui_id, item_gui_id, tree_obj.tree_nodes[i].name, tree_obj.tree_nodes[i].type, tree_obj.tree_nodes[i].eval);
  }                            
...
function lib_tree_print_item(parent_gui_id, item_gui_id, itemName, itemType, my_eval)
{
  // HTML-Code :
  // <LI ...>
  //   <IMG ... /IMG>
...
{    
  ...
  if (is_new)
  {
    ...
    this.print_item(parent_gui_id, my_gui_id, "", item_type, this.empty_eval_struct);
  }
